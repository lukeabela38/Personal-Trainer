...
export async function loadIndex() {
  ...
  const resp = await fetch(
    `${HISTORY_BASE.pathname}/index.json?v=${Date.now()}`,
  );
...
export async function loadSnapshot(dateStr) {
  ...
  const resp = await fetch(
    `${HISTORY_BASE.pathname}/${dateStr}.json?v=${Date.now()}`,
  );
...
export function weeklySummary(snapshots) {
  ...
  const hardDays = priorities.filter((p) =>
    [
      "aerobic_quality",
  ...
  const recoveryDays = priorities.filter((p) =>
    ["recovery", "nutrition_repair"].includes(p),
  ).length;

  const cals = weekDays.map((s) => s.cronometer?.today?.calories_consumed ?? 0);
  const avgCals = cals.length
  ...
  return {
    ...
    vo2: vo2 ?? null,
    vo2Delta:
      vo2 != null && vo2Prev != null ? (vo2 - vo2Prev).toFixed(1) : null,
    bw: bw ?? null,
