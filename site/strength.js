...
function renderTrendModal(name, history) {
  const orderedHistory = [...history].sort((a, b) =>
    String(a?.date ?? a?.workout_start_time ?? "").localeCompare(
      String(b?.date ?? b?.workout_start_time ?? ""),
    ),
  );
  const oneRms = orderedHistory
    .map((item) => item.estimated_one_rm_kg)
    .filter((value) => value != null);
  const weights = orderedHistory
    .map((item) => item.weight_kg)
    .filter((value) => value != null);
  const reps = orderedHistory
    .map((item) => item.reps)
    .filter((value) => value != null);
  ...
  const modalWarmup =
    latest?.weight_kg != null && latest.weight_kg > 0
      ...
      : null;
  const latestReps = parseInteger(latest?.reps);
  const modalRest =
    latestReps != null
      ? latestReps <= 5
        ? "3-5 min"
  ...
  const recent = orderedHistory.slice(-10).reverse();
